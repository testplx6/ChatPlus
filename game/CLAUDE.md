...
| fichier | quoi |
|---|---|
| `METHODE.md` | comment on travaille ici — chaque règle avec l'incident qui l'a écrite ; **§11 la lenteur** : les sept règles de la chasse d'août 2026, et ce que chacune a coûté ; **§12 la mesure ment aussi** : une somme d'unités hétérogènes n'est pas une mesure, et un invariant vert est une information |
...
| `PACTES.md` | les pactes entre drapeaux — **ouvert** : pas un menu d'alliances toutes faites mais un système de clauses qu'on propose, qu'on accepte si l'on y trouve son compte, et qu'on tient ou trahit. **P1 et P2 livrés** : les pactes existent et les deux clauses qui comptent mordent — ne pas s'attaquer, se porter secours (et manquer par impuissance n'est pas manquer par choix). **P3 reste retiré, mais son procès est refait** : la « masse monétaire qui quadruple » était un **artefact de mesure** — on additionnait des unités de pays dont les cours vont de 0,01 à 186. En valeur réelle (colonne « en crédit », neuve au banc), le monde de P3 vaut 3,48 M contre 2,79 au témoin, et le cas qui paraissait le pire valait exactement le témoin. Il ne reste qu'un défaut franc — un cours à 186 —, et il se traite dans `monnaie.js`, pas dans les pactes |
