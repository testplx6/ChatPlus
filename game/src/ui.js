...
function blocSituation() {
  ...
  for (const m of menacesSurLaBase(S)) {
    const t = `${FACTIONS[m.faction].nom} marche sur ${S.base.nom} — ${m.vue
...
function blocColonie(col) {
  ...
  <section class="panneau">
    ...
    ? 'ville libre' : e(FACTIONS[col.faction].nom)}</span></h2>
    <div class="grille2">
...
function blocCibleOrdre(o) {
  ...
  if (o.type === 'ravitaillement') {
    ...
    if (chez) cible = { regionId: chez.i, quoi: `leurs terres les plus proches` };
      ...
      return `<div class="aide">On ne connaît aucune terre ${e(FACTIONS[o.cibleFaction].genitif)}
...
function blocOuVousEnEtes() {
  ...
  const lignes = rangs.map(({ k, ef }) => {
    ...
    return `<div class="ligne souple">
      <span class="k" style="color:${couleurFaction(k)}">${e(FACTIONS[k].nom)}<br>
