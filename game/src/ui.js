...
function ecranBase() {
  ...
  const peutPlanter = !rIci.colonie && !camps.some((c) => c.fonde && c.regionId === G().regionId)
    && Object.keys(COUT_FONDATION).every((k) => (G().inventaire[k] || 0) >= COUT_FONDATION[k]);
  const campsHtml = camps.length > 1 || peutPlanter ? `<section class="panneau pliable">
    <h2 class="titre">Vos camps <span class="droite">${camps.filter((c) => c.fonde).length}</span></h2>
    <div class="pile">
      ${camps.map((c, i) => {
    const ici = i === (S.campActif || 0);
    const loin = c.fonde && c.regionId != null
      ? distance(c.regionId, G().regionId) : null;
    return `<button class="act ${ici ? '' : 'mini'}" data-a="camp" data-i="${i}"
      ...
      ${peutPlanter ? `<button class="act primaire" data-a="fonder">
        Planter un camp de plus, ici</button>` : ''}
    </div>
    <div class="aide">On en tient autant qu’on veut. Celui qu’on quitte continue de
    vivre — ses gens mangent, ses chaînes tournent, son entrepôt se remplit.</div>
  </section>` : '';
...
function surClic(ev) {
  ...
  switch (a) {
    ...
    // Habiter un autre camp (M4). Le rendu suit tout seul : `S.base` est une
    // référence, et l'écran lit `S.base`.
    case 'camp': {
      const r = ACTIONS.changerDeCamp(Number(el.dataset.i));
      if (!r.ok) replique(r.motif);
      else replique(`Vous êtes à ${r.camp.nom || 'votre camp'}.`);
      break;
    }
