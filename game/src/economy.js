...
export function acheter(state, col, key, qte, groupe) {
  const g = groupe || groupeActif(state);
  if (!col || col.ruine) return { ok: false, motif: 'Il n’y a plus personne ici.', qte: 0, cout: 0 };
...
export function vendre(state, col, key, qte, groupe) {
  const g = groupe || groupeActif(state);
  // On ne commerce pas avec une ruine, et c'est par là que fuyaient les
  // comptes : l'audit a le droit de ne pas compter une ville morte
  // (`effondrer` vide ses caisses — « laisser les comptes garnis sur une
  // ruine, c'était un avoir fantôme »), mais rien n'empêchait d'y vendre
  // ENSUITE. L'argent entrait dans une caisse que personne ne compte plus.
  // Trouvé en remontant un écart de 586 crédits sur seize mille heures : la
  // caisse d'une ruine était montée de 124 à 586, au crédit près.
  //
  // Le garde est ici et non dans `encaisser` : là-bas il romprait la paire
  // `encaisser` + `entrerDehors`, et la masse monterait sans contrepartie —
  // essayé, mesuré, l'écart avait doublé.
  if (!col || col.ruine) return { ok: false, motif: 'Il n’y a plus personne ici.', qte: 0, gain: 0 };
