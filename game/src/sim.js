...
export function nouvellePartie(seed, opts = {}) {
  ...
  // On part avec rien.
  //
  // L'escouade démarrait armée, cuirassée, avec quarante-cinq rations, vingt
  // ferrailles, deux medkits et quatre cent cinquante crédits — au motif
  // qu'« une bande de pillards liquide une escouade sans armure avant qu'elle
  // ait appris quoi que ce soit ». C'est vrai, et c'est précisément le sujet :
  // ou l'on meurt vite et l'on recommence, ou l'on s'en sort et l'on garde ce
  // qu'on a arraché. Un début confortable rend les cent premières heures sans
  // enjeu, et le reste de la partie n'a plus rien à raconter d'elles.
  //
  // Un peu d'aléa, aussi : trois parties qui commencent au caractère près, ce
  // n'est pas un monde, c'est un niveau.
  //
  // Et l'on commence **seul**. Une escouade de trois, c'est déjà une compagnie :
  // on encaisse une embuscade, on porte quelqu'un, on se répartit les métiers.
  // Seul, chaque décision compte double et le premier compagnon qu'on recrute
  // est un événement. Le mot « escouade » se mérite.
  ...
  //
  // `opts.equipe` sert au harnais, qui vérifie des mécaniques de groupe — porter
  // un blessé, répartir les tâches, manger ses morts — et qui aurait besoin de
  // recruter trois personnes avant chaque test sans ça.
  // Un départ en ville est un départ *installé* : l'ancien paquetage complet,
  // armes et armures comprises. C'est ce dont vivent le banc et le harnais, qui
  // ne jouent pas la partie — ils la font tourner. Une escouade désarmée et sans
  // vivres que personne ne dirige meurt en six cents heures, et l'on mesurerait
  // alors la famine plutôt que ce qu'on cherche.
  const combien = Math.max(1, opts.equipe || (enVille ? 3 : 1));
  if (enVille) {
    const paquetage = [
    ...
    for (let i = 0; i < combien; i++) {
      ...
      // valeur du métier. On désarme donc après coup, explicitement : celui qui
      // se réveille dans la poussière n'a rien, pas même ce que son ancien
      // travail lui aurait laissé entre les mains.
      c.equip.arme = null;
      c.equip.armure = null;
    ...
    // Et l'on n'était pas seul en arrivant.
    //
    // Testé en jouant : le départ sauvage n'est pas dur, il est *lent*. On
    // ramasse tranquillement pendant deux jours avant que quoi que ce soit
    // arrive, et rien ne demande de décision. Ce n'est pas de la difficulté qui
    // manque, c'est une situation.
    //
    // Alors on se réveille à côté de celui avec qui on voyageait. Il est mort,
    // il a une arme et une armure, et la colonne le porte tant qu'on n'a rien
    // décidé — ce qui ralentit la marche et pèse sur le moral (voir
    // depouilles.js). La première décision de la partie tombe donc avant le
    // premier pas, elle n'est pas confortable, et elle est déjà un peu de ce
    // qu'on va devenir : l'enterrer et repartir les mains vides, ou le
    // dépouiller et marcher armé.
  ...
  const state = {
    ...
    world,
    player: {
      credits: enVille ? 450 : rng.irange(20, 70),
