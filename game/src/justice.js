...
/**
 * Ce qu'on peut prendre après une victoire. On ne capture pas les morts, et l'on
 * ne capture jamais l'Essaim — on ne négocie pas avec ça, et personne n'en veut.
 *
 * Rien n'interdit d'en prendre plus qu'on ne sait en garder. C'était le cas
 * jusqu'ici — `capturables` refusait net au-delà de la capacité — et c'était une
 * limite écrite, exactement ce que ce projet évite partout ailleurs : on peut
 * acheter une dixième bête, bâtir un camp trop grand, porter trop lourd. Le
 * modèle avait déjà le coût qui borne naturellement le nombre — ils mangent
 * (`tickPrisonniers`), ils ralentissent (`lenteurPrisonniers`), et ceux que
 * personne ne regarde s'évadent (`surveillanceManquante`) — mais on ne pouvait
 * jamais entrer dans le régime où il s'applique.
 *
 * Conséquence mesurée : la voie du négrier n'avait aucun moyen de grandir. Une
 * escouade de quatre gardait six captifs, pour toujours, quoi qu'elle achète ou
 * apprenne. Toutes les autres voies montent en charge ; celle-là était plate.
 */
export function capturables(g, bande) {
  if (!bande || bande.faction === 'essaim') return [];
  return (bande.membres || []).filter((c) => estVivant(c) && !estDebout(c));
}
 ...
 *
 * « Beaucoup » a longtemps été faux. À 1,9 fois la valeur, un homme se vendait
 * 360 crédits — moins qu'une charrette à bras, trois fois moins qu'une brahmine.
 * Mesuré au banc, la voie du négrier rapportait 4 750 crédits par partie et en
 * coûtait près de 8 000 en travail non fait, en soins, en matériel perdu et en
 * escouades éteintes : elle était strictement dominée par le travail honnête,
 * ce qui n'est pas un choix moral, c'est un piège. Le coût dominant n'est pas
 * l'estime perdue, c'est l'heure passée à chasser plutôt qu'à récolter.
 *
 * À 3,2, une prise paie mieux que l'heure qu'elle a coûtée — et le risque, les
 * morts et le mépris redeviennent ce qui décide, au lieu d'être un supplément
 * à une mauvaise affaire.
 */
export function prixEsclave(state, col, c) {
  ...
  return Math.round(valeurCaptif(c) * (loi.sansLoi ? 2.3 : 3.2));
}
...
export function disposer(state, g, captifId, quoi, log) {
  ...
  if (quoi === 'relacher') {
    ...
    noterReputation(state, cap.faction, 4, col ? col.regionId : null);
